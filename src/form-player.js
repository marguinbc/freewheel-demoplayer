...
tv.freewheel.DemoPlayer = function() {
  ...
  // Setup default values for FW test ad campaigns
  ...
  playPauseBtn.onclick = function() {
    ...
    if (player.paused) {
      if (firstPlay) {
        if (fwPlayer.adResponseLoaded) {
          fwPlayer.play();
        } else {
          fwPlayer.requestAds();
          setTimeout(fwPlayer.play.bind(fwPlayer), 500);
        }
        firstPlay = false;
      } else {
        player.play();
...
tv.freewheel.DemoPlayer.prototype = {
  requestAds: function() {
    ...
    var formNetworkId = document.getElementById('networkID').value;
    var formSiteSectionId = document.getElementById('siteSectionID').value;
    var formVideoAssetId = document.getElementById('videoAssetID').value;
    var formProfile = document.getElementById('profile').value;
    var formServer = document.getElementById('server').value;

    this.networkId =  formNetworkId ? formNetworkId : this.networkId;
    this.siteSectionId = formSiteSectionId ? formSiteSectionId : this.siteSectionId;
    this.videoAssetId = formVideoAssetId ? formVideoAssetId : this.videoAssetId;
    this.profile = formProfile ? formProfile : this.profile;
    this.server = formServer ? formServer : this.server;
    ...
    */
    ...
    // Add preroll slots if specified in form, otherwise use default single slot
    var formPrerollSlots = parseInt(document.getElementById('prerollQty').value);
    if (formPrerollSlots > 0) {
      for (var i=1; i <= formPrerollSlots; i++) {
        var slotName = "Preroll_" + i;
        this.currentAdContext.addTemporalSlot(slotName, tv.freewheel.SDK.ADUNIT_PREROLL, 0);
      }
    } else {
      this.currentAdContext.addTemporalSlot("Preroll_1", tv.freewheel.SDK.ADUNIT_PREROLL, 0);
    }

    // Add pre, mid and postroll slots
    // Uses 1 preroll, 3 midroll and 1 postroll by default

    var formMidrollSlots = parseInt(document.getElementById('midrollQty').value);
    if (formMidrollSlots > 0) {
      for (var i=1; i <= formMidrollSlots; i++) {
        var slotName = "Midroll_" + i;
        var slotTimePos = document.getElementById('midrollTimePos').value.split(',');
        if (slotTimePos.length === formMidrollSlots) {
          this.currentAdContext.addTemporalSlot(slotName, tv.freewheel.SDK.ADUNIT_MIDROLL, slotTimePos[i-1]);
        } else {
          console.log('ERROR: No midroll slots added - number of time positions must match number of midrolls specified');
        }
      }
    } else {
    ...
    var formPostrollSlots = parseInt(document.getElementById('postrollQty').value);
    if (formPostrollSlots > 0) {
      for (var i=1; i <= formPostrollSlots; i++) {
        var slotName = "Postroll_" + i;
        var slotTimePos = fwPlayer.videoEl.duration;
        this.currentAdContext.addTemporalSlot(slotName, tv.freewheel.SDK.ADUNIT_POSTROLL, slotTimePos );
      }
    } else {
